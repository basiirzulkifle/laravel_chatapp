    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ml') ? 'is-invalid' : '' }}" type="text" name="ml" id="ml" value="{{ old('ml', $user->ml) }}" required>
                @if($errors->has('ml'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('game_rank') ? 'is-invalid' : '' }}" name="game_rank" id="game_rank" required>
                    ...
                    @foreach(App\Models\User::GAME_RANK_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('game_rank', $user->game_rank) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('game_role') ? 'is-invalid' : '' }}" name="game_role" id="game_role">
                    ...
                    @foreach(App\Models\User::GAME_ROLE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('game_role', $user->game_role) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
